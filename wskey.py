...
def get_wskey():  # 方法 获取 wskey值 [系统变量传递]
    ...
    for i in range(len(envlist)):  # For循环 变量[envlist]的数量
        if "name" in envlist[i] and envlist[i]["name"] == "DDDD":  # 判断 envlist内容
            ...
            continue  # 继续循环
    if "DDDD" in os.environ:  # 判断 DDDD是否存在于环境变量
        if len(wskey_list) > 0:  # 判断 WSKEY 数量 大于 0 个
            return wskey_list  # 返回 WSKEY [LIST]
        else:  # 判断分支
            logger.info("DDDD变量未启用")  # 标准日志输出
            sys.exit(1)  # 脚本退出
    else:  # 判断分支
        logger.info("未添加DDDD变量")  # 标准日志输出
        sys.exit(0)  # 脚本退出
